...
public class DecimalUtilsTest {
	...
	final Decimal[] longs = DecimalUtils.packLongs(23142341234123412L,
		345235234523532453L, new Date().getTime());
	assertEquals(3, longs.length);
    }

    @Test
    public void testPackDoubles() {
	final Decimal[] doubles = DecimalUtils.pack(123.456, 5635.32353,
		6754353.4234);
	...
	final List<Decimal> longs = DecimalUtils.packLongList(
		23142341234123412L, 345235234523532453L, new Date().getTime());
	assertEquals(3, longs.size());
    }

    @Test
    public void testPackDoubleList() {
	final List<Decimal> doubles = DecimalUtils.packList(123.456,
		5635.32353, 6754353.4234);
	assertEquals(3, doubles.size());
    }

    @Test
    public void testUltraPack() {
	final Decimal[] doubles = DecimalUtils.$$(123.456, 5635.32353,
		6754353.4234);
	assertEquals(3, doubles.length);
    }
    ...
    /**
     * fails with a HashSet, since it relies on equals(), which is different for
     * ints and doubles
     */
    @Test
	...
	// assertTrue(DecimalUtils.containsAll(decimals,
	// Arrays.asList(DecimalUtils.pack(1,2,3))));
    }

    // an array of ints, is not equal to the array of doubles, because
    // new BigDecimal(1d).equals(new BigDecimal(1) is false since the double
    // value is 1.0d

    @Test
    public void testThatAListOfDecimalCanContainAllTheElementsOfAnotherListIfTheyAreTheSame() {
	final Decimal[] doubles = DecimalUtils.pack(1, 2, 1, 2, 3, 4, 3, 4, 5,
		6, 5, 6, 7, 8, 7, 8);
	final List<Decimal> doublesList = Arrays.asList(doubles);
	final List<Decimal> intsList = Arrays.asList(DecimalUtils.packInts(1,
		2, 3, 4, 5, 6, 7, 8));
	...
	final List<Decimal> doublesList = Arrays.asList(DecimalUtils.pack(1, 2,
		1, 2, 3, 4, 3, 4, 5, 6, 5, 6, 7, 8, 7, 8));
	final Set<Decimal> doublesSet = new TreeSet<Decimal>(doublesList);
	final Set<Decimal> intsSet = new TreeSet<Decimal>(Arrays
		.asList(DecimalUtils.packInts(1, 2, 3, 4, 5, 6, 7, 8)));
