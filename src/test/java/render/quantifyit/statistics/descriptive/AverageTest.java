...
public class AverageTest {
	...
	final double[] dataset = new double[] { 1.3, 2.5, 3.1, 4.25, 5.97,
		6.41, 7.132 };
	...
	final double[] dataset = new double[] { 1, 2, 3, 2, 4, 6, 4, 5, 1, 3,
		2, 1, 5, 1, 2, 3 };
	final Decimal[] elements = pack(dataset);
	...
	final Decimal[] elements = pack(1, 2, 1, 2, 3, 4, 3, 4, 5, 6, 5, 6, 7,
		8, 7, 8);
	final Decimal[] modes = Average.mode(elements);
	assertEquals(8, modes.length);
	final List<Decimal> results = Arrays
		.asList(pack(1, 2, 3, 4, 5, 6, 7, 8));
	assertTrue(Arrays.asList(modes).containsAll(results));
    }
