...
public class DivideTest {
	...
	final Decimal result = new Divide(312, 7).halfEven().precision(4)
		.eval();
	...
	assertDecimal(Division
		.divide(Decimal.$(Integer.MAX_VALUE), Decimal.TEN), largeInt);
    }
	...
	final Decimal byDoubleContext = new Divide(700L, 3L).precision(2)
		.eval();
	assertDecimal(230, byDoubleContext);

	final Decimal largeLong = new Divide(Long.MAX_VALUE, 10L).eval();
	assertDecimal(Division.divide(Decimal.$(Long.MAX_VALUE), Decimal.TEN),
		largeLong);
    }
	...
	final Decimal byDoubleContext = new Divide(700d, 3d).precision(2)
		.eval();
	assertDecimal(230, byDoubleContext);

	final Decimal largeDouble = new Divide(Double.MAX_VALUE, 10d).eval();
	assertDecimal(
		Division.divide(Decimal.$(Double.MAX_VALUE), Decimal.TEN),
		largeDouble);
    }
	...
	final BigDecimal expected = new BigDecimal(312d).divide(new BigDecimal(
		7d), RoundingMode.HALF_EVEN);
	...
	final BigDecimal expected = new BigDecimal(312d).divide(new BigDecimal(
		7d), 10, RoundingMode.HALF_EVEN);
	assertEquals(new BigDecimal(44.5714285714, new MathContext(12)),
		expected);
	...
	BigDecimal bigDecimalApproximation = new BigDecimal(223d).divide(
		new BigDecimal(71d), RoundingMode.HALF_EVEN);
	assertEquals(new BigDecimal(3), bigDecimalApproximation);

	bigDecimalApproximation = new BigDecimal(223d).divide(new BigDecimal(
		71d), 5, RoundingMode.HALF_EVEN);
	...
	BigDecimal bigDecimalApproximation = new BigDecimal(355d).divide(
		new BigDecimal(113d), RoundingMode.HALF_EVEN);
	assertEquals(new BigDecimal(3), bigDecimalApproximation);

	bigDecimalApproximation = new BigDecimal(355d).divide(new BigDecimal(
		113d), 7, RoundingMode.HALF_EVEN);
