...
public final class AssertDecimal {
    ...
    public static void assertDecimal(final Decimal expected,
	    final Decimal actual) {
	assertTrue(
		String
			.format(
				"Decimal comparison: expected:%n%s but got: [%n%s%nPrecision: %s%nScale: %s]",
				expected, actual, actual.getPrecision(), actual
					.getScale()), expected.same(actual));
    }
    ...
    public static void assertDecimal(final Decimal expected,
	    final Decimal actual, final int scale,
    ...
    public static void assertDecimal(final double expected,
	    final Decimal actual, final int scale,
	    final RoundingMode roundingMode) {
	assertDecimal($(expected), actual, scale, roundingMode);
    }
