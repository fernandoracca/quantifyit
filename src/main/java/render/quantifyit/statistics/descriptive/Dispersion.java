...
public final class Dispersion {
    ...
    public static Decimal sampleVariance(final Decimal mean,
	    final Decimal[] elements) {
    ...
    public static Decimal populationVariance(final Decimal mean,
	    final Decimal[] elements) {
	if (elements.length == 1) {
	    return Decimal.ZERO;
	}
    ...
    public static Decimal sampleStandardDeviation(final Decimal mean,
	    final Decimal... elements) {
    ...
    public static Decimal populationStandardDeviation(final Decimal mean,
	    final Decimal... elements) {
	...
	if (variance.isNegative()) {
	    throw new IllegalArgumentException(
		    "Please give a non negative number.");
	}
    ...
    public static Decimal zScore(final Decimal element,
	    final Decimal populationMean,
	    final Decimal populationStandardDeviation) {
     ...
     * ∑ (x - μ)^2
     */
    private static Decimal sumOfSquaredDeltas(final Decimal mean,
	    final Decimal... elements) {
	Decimal sumOfSquaredDeltas = Decimal.ZERO;
	for (Decimal element : elements) {
	    sumOfSquaredDeltas = sumOfSquaredDeltas.plus(element.minus(mean)
		    .square());
	}
